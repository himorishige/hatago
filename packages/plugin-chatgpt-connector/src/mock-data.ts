...
export const mockDocuments: Document[] = [
  {
    ...
    content: `Hatago is a lightweight, fast, and simple remote MCP (Model Context Protocol) server built with Hono + @hono/mcp + MCP TypeScript SDK. It features a plugin-based architecture for extensibility and runs on Node.js, Cloudflare Workers, Deno, and Bun.`,
    url: 'https://docs.hatago.dev/overview',
  },
  {
    id: 'mcp-002',
    title: 'MCP Protocol Overview',
    content: `The Model Context Protocol (MCP) is an open protocol that enables seamless integration between LLM applications and external data sources and tools. It provides a standardized way for AI assistants to access context and take actions through tool functions.`,
    url: 'https://docs.hatago.dev/mcp-protocol',
  },
  {
    id: 'plugin-003',
    title: 'Creating Hatago Plugins',
    content: `Hatago plugins follow the HatagoPlugin type pattern: (ctx: HatagoContext) => void | Promise<void>. The context provides access to the Hono app instance for HTTP routes and the MCP server instance for registering tools and resources.`,
