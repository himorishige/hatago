...
import type { ExampleResult, LogEntry, TestScenario } from './types.js'
  ...
  if (typeof data === 'object' && data !== null) {
    return Object.fromEntries(
      Object.entries(data).map(([key, value]) => [
        key,
        // 機密情報のキーをマスクする
        /password|secret|token|key|auth/i.test(key) ? '***MASKED***' : maskSensitiveData(value),
      ])
    )
  }
