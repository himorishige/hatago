...
async function makeRequest(url: string): Promise<{ success: boolean; latency: number; error?: string }> {
...
async function runLoadTest(config: LoadTestConfig): Promise<LoadTestResult> {
  ...
  // Start workers
  const workers = Array(Math.min(config.concurrency, 10)).fill(0).map(() => worker())
...
async function runAllLoadTests() {
  ...
  try {
    ...
    for (const { name, result } of results) {
      ...
      const successRate = `${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`.padStart(12)
      const errorCount = result.failedRequests.toString().padStart(6)
      
      console.log(`| ${testName} | ${rps} | ${avgLat} | ${p95Lat} | ${successRate} | ${errorCount} |`)
    }
    console.log('-'.repeat(100))
    ...
    const targets = [
      ...
      { name: 'P95 Latency (< 50ms)', target: 50, actual: results[0].result.p95Latency, reverse: true },
      { name: 'Success Rate (> 99%)', target: 99, actual: (results[0].result.successfulRequests / results[0].result.totalRequests) * 100 },
    ]
    ...
    for (const check of targets) {
      ...
      console.log(`${status} ${check.name}: ${check.actual.toFixed(1)}${unit} (target: ${check.reverse ? '<=' : '>='} ${check.target}${unit})`)
    }
