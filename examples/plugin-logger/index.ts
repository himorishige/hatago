...
import type { ExampleConfig, TestScenario, LogEntry } from '../_shared/types.js'
 ...
 * ログレベルフィルター（高階関数）
 */
const createLevelFilter = (minLevel: LogLevel): LogFilter => 
  (entry) => getLogLevelPriority(entry.level) >= getLogLevelPriority(minLevel)

/**
 * サンプリングフィルター（確率的フィルタリング）
 */
const createSampleFilter = (sampleRate: number): LogFilter => 
  (entry) => {
    // エラーレベル以上は必ず通す
...
const createLogPipeline = (config: LoggerConfig) => {
  ...
  const filter = (entry: LogEntryWithMetadata): boolean => {
    return createLevelFilter(config.level)(entry) && 
           createSampleFilter(config.sampleRate)(entry)
  }
...
const writeLog = (formattedLog: string, level: LogLevel): void => {
  // レベルに応じた出力先選択
  const output = getLogLevelPriority(level) >= getLogLevelPriority('error') 
    ? console.error 
    : console.log
 ...
 * ログクエリツールの実装
 */
const createLogsQueryTool = (logStore: LogEntryWithMetadata[]) => 
  async (request: any) => {
    const { level, since, limit = 100, search } = request.params.arguments as {
      level?: LogLevel
      since?: string
      limit?: number
      search?: string
    ...
    if (level) {
      ...
      filtered = filtered.filter(entry => 
        getLogLevelPriority(entry.level) >= minPriority
      )
    }
 ...
 * ログ設定ツールの実装
 */
const createLogsConfigTool = (config: LoggerConfig, updateConfig: (newConfig: Partial<LoggerConfig>) => void) =>
  async (request: any) => {
...
export const createLoggerPlugin = (
  ...
  return async (ctx) => {
    ...
    // MCPツール登録
    ctx.server.registerTool('logs_query', {
    ...
    ctx.server.registerTool('logs_config', {
