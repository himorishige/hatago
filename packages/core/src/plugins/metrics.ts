...
export const metrics: HatagoPluginFactory<MetricsConfig> =
  ...
  ({ app }) => {
    ...
    app.get(endpoint, c => {
      ...
      const durations = metrics.request_duration_histogram.map(r => r.duration).sort((a, b) => a - b)
      if (durations.length > 0) {
        ...
        lines.push(`request_duration_ms_bucket{le="10"${histogramKey.slice(1, -1)}} ${durations.filter(d => d <= 10).length}`)
        lines.push(`request_duration_ms_bucket{le="50"${histogramKey.slice(1, -1)}} ${durations.filter(d => d <= 50).length}`)
        lines.push(`request_duration_ms_bucket{le="100"${histogramKey.slice(1, -1)}} ${durations.filter(d => d <= 100).length}`)
        lines.push(`request_duration_ms_bucket{le="500"${histogramKey.slice(1, -1)}} ${durations.filter(d => d <= 500).length}`)
        lines.push(`request_duration_ms_bucket{le="+Inf"${histogramKey.slice(1, -1)}} ${durations.length}`)
        lines.push(`request_duration_ms_sum${histogramKey} ${durations.reduce((sum, d) => sum + d, 0)}`)
