...
async function createApp(options = {}) {
  ...
  app.get("/health", (c) => c.json({ ok: true, name, version, timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
...
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
var helloHatago = /* @__PURE__ */ __name(() => ({ server }) => {
  server.registerTool(
    "hello.hatago",
    {
      title: "Hello Hatago",
    ...
    async (_args, extra) => {
      const text = "Hello Hatago! This is a test string with approximately 100 characters for progress notification.";
...
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
var oauthMetadata = /* @__PURE__ */ __name((opts = {}) => ({ app, getBaseUrl }) => {
  ...
  if (opts.requireAuth) {
    app.use("/mcp", async (c, next) => {
      ...
      if (!hasBearer) {
        const base = opts.resource ? new URL(opts.resource) : getBaseUrl(c.req.raw);
        c.header("WWW-Authenticate", `Bearer resource_metadata="${new URL("/.well-known/oauth-protected-resource", base.origin).toString()}"`);
